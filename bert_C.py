...
if __name__=='__main__':
    ...
    print('data_load')
    with open('dataC.pkl', 'rb') as p:
        data_list = pickle.load(p)
    with open('dataC_dict.pkl', 'rb') as p:
        num_to_chr = pickle.load(p)
    ...
    # 모델 로컬 저장
    torch.save(model, f'./result_m_C.pt')
    torch.save(model.state_dict(), f'./result_state_dict_C.pt')  # 모델 객체의 state_dict 저장
    torch.save({
        'model': model.state_dict(),
        'optimizer': optimizer.state_dict()
    }, f'./result_C.tar')  # 여러 가지 값 저장, 학습 중 진행 상황 저장을 위해 epoch, loss 값 등 일반 scalar값 저장 가능
